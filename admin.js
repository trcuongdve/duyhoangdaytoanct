...
async function renderClasses() {
  ...
  allNames.forEach((cls, idx)=>{
    ...
    card.innerHTML = `
      ...
      <div style="background:${color};padding:1.1rem 1.25rem;display:flex;align-items:center;justify-content:space-between">
        ...
        <div style="display:flex;gap:.3rem">
          ...
          <button class="btn-sm" data-selective="${cls}" title="Xóa chọn lọc" style="background:rgba(255,165,0,.35);border-color:rgba(255,255,255,.25);color:#fff;font-size:.8rem" onclick="event.stopPropagation()">🎯</button>
    ...
    card.addEventListener('click', e=>{ if(!e.target.closest('[data-edit],[data-del],[data-selective]')) openClassDetail(cls); });
    card.querySelector('[data-edit]').addEventListener('click', e=>{ e.stopPropagation(); openEditClassModal(cls, info); });
    card.querySelector('[data-selective]').addEventListener('click', e=>{ e.stopPropagation(); openDeleteSelectiveModal(cls); });
...
// ============================================================
// XÓA HỌC SINH CÓ CHỌN LỌC THEO LỚP
// Logic: KHÔNG TÍCH = xóa ngay | TÍCH = giữ lại (có thể đặt ngày xóa riêng)
// ============================================================
(function initDeleteSelectiveModal() {
  let _dsClass = '';
  let _dsAllStudents = [];
  let _dsKeepIds = new Set();   // id học viên ĐƯỢC GIỮ (đã tích)
  let _dsExpireDates = {};      // { [id]: Date | null } — ngày xóa tương lai cho người được giữ
  let _dsTimers = [];

  function isKept(sid) { return _dsKeepIds.has(String(sid)); }

  // ---- Mở modal ----
  window.openDeleteSelectiveModal = async function(cls) {
    _dsClass = cls;
    _dsKeepIds = new Set();
    _dsExpireDates = {};
    _dsTimers.forEach(t => clearTimeout(t));
    _dsTimers = [];

    const { data: scList } = await db.from('student_classes').select('student_id').eq('class_name', cls);
    const scIds = (scList || []).map(s => s.student_id);
    const { data: fallback } = await db.from('students').select('id').eq('class_name', cls);
    const fallbackIds = (fallback || []).map(s => s.id).filter(id => !scIds.includes(id));
    const allIds = [...new Set([...scIds, ...fallbackIds])];

    let students = [];
    if (allIds.length) {
      const { data } = await db.from('students').select('id,full_name,username,student_code').in('id', allIds).order('full_name');
      students = data || [];
    }
    _dsAllStudents = students;

    document.getElementById('dsModalClassName').textContent = cls;
    document.getElementById('dsModalTotalCount').textContent = students.length;
    ...
    renderDsStudentList('');
    updateDsCount();
    document.getElementById('deleteSelectiveModal').classList.add('open');
  };

  // ---- Render hàng ----
  function rowHtml(s) {
    const sid = String(s.id);
    const kept = isKept(sid);
    const dateVal = _dsExpireDates[sid] ? new Date(_dsExpireDates[sid]).toISOString().slice(0,16) : '';
    return `<div class="ds-row" data-sid="${sid}"
      style="display:flex;align-items:center;gap:.75rem;padding:.6rem 1rem;border-bottom:1px solid var(--border);${kept ? 'background:#f0fdf4' : 'background:#fff5f5'}">
      <input type="checkbox" class="ds-cb" data-id="${sid}" ${kept ? 'checked' : ''}
        style="flex-shrink:0;cursor:pointer;width:18px;height:18px;accent-color:#6366f1;margin:0;padding:0;border:none">
      <div style="flex:1;min-width:0">
        <div style="font-weight:600;font-size:.87rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${s.full_name || '—'}</div>
        <div style="font-size:.73rem;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${s.username || ''}</div>
      </div>
      <div class="ds-date-cell" style="width:155px;flex-shrink:0">
        ${kept
          ? `<input type="datetime-local" class="ds-date-input" data-id="${sid}" value="${dateVal}"
              title="Ngày xóa (để trống = giữ mãi mãi)"
              style="padding:.3rem .45rem;border:1.5px solid #6ee7b7;border-radius:7px;outline:none;font-size:.76rem;color:#065f46;background:#f0fdf4;width:100%">`
          : '<span style="color:var(--muted);font-size:.78rem">Xóa ngay</span>'}
      </div>
      <div class="ds-badge-cell" style="flex-shrink:0;min-width:72px;text-align:right">
        ${kept
          ? '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#d1fae5;color:#065f46;white-space:nowrap">✓ Giữ lại</span>'
          : '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#fee2e2;color:#991b1b;white-space:nowrap">🗑 Xóa</span>'}
      </div>
    </div>`;
  }

  function renderDsStudentList(query) {
    const container = document.getElementById('dsStudentList');
    const filtered = query
      ? _dsAllStudents.filter(s =>
          (s.full_name || '').toLowerCase().includes(query) ||
          (s.username || '').toLowerCase().includes(query) ||
          (s.student_code || '').toLowerCase().includes(query))
      : _dsAllStudents;
    if (!filtered.length) {
      container.innerHTML = '<div style="padding:1.25rem;text-align:center;color:var(--muted);font-size:.85rem">Không tìm thấy học viên nào.</div>';
      return;
    }
    container.innerHTML = filtered.map(rowHtml).join('');
  }

  // ---- Click delegation ----
  document.getElementById('dsStudentList').addEventListener('click', e => {
    if (e.target.closest('.ds-date-input')) return;
    const row = e.target.closest('.ds-row');
    if (!row) return;
    const id = row.dataset.sid; if (!id) return;
    const cb = row.querySelector('.ds-cb'); if (!cb) return;
    if (e.target !== cb) cb.checked = !cb.checked;
    const kept = cb.checked;

    if (kept) _dsKeepIds.add(id); else { _dsKeepIds.delete(id); delete _dsExpireDates[id]; }
    row.style.background = kept ? '#f0fdf4' : '#fff5f5';

    const dc = row.querySelector('.ds-date-cell');
    const existingVal = _dsExpireDates[id] ? new Date(_dsExpireDates[id]).toISOString().slice(0,16) : '';
    dc.innerHTML = kept
      ? `<input type="datetime-local" class="ds-date-input" data-id="${id}" value="${existingVal}"
          title="Ngày xóa (để trống = giữ mãi mãi)"
          style="padding:.3rem .45rem;border:1.5px solid #6ee7b7;border-radius:7px;outline:none;font-size:.76rem;color:#065f46;background:#f0fdf4;width:100%">`
      : '<span style="color:var(--muted);font-size:.78rem">Xóa ngay</span>';

    row.querySelector('.ds-badge-cell').innerHTML = kept
      ? '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#d1fae5;color:#065f46;white-space:nowrap">✓ Giữ lại</span>'
      : '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#fee2e2;color:#991b1b;white-space:nowrap">🗑 Xóa</span>';

    updateDsCount(); renderDsPreview();
  });

  document.getElementById('dsStudentList').addEventListener('change', e => {
    const di = e.target.closest('.ds-date-input');
    if (di) { _dsExpireDates[di.dataset.id] = di.value ? new Date(di.value) : null; renderDsPreview(); }
  });

  function updateDsCount() {
    const kept = _dsKeepIds.size;
    const del  = _dsAllStudents.length - kept;
    const el = document.getElementById('dsSelectedCount');
    if (el) el.textContent = `${kept} giữ lại · ${del} sẽ xóa`;
  }

  function renderDsPreview() {
    const preview = document.getElementById('dsPreview');
    const toDeleteNow = _dsAllStudents.filter(s => !isKept(s.id));
    const toDeleteLater = _dsAllStudents.filter(s => isKept(s.id) && _dsExpireDates[String(s.id)] && _dsExpireDates[String(s.id)] > new Date());
    if (!_dsAllStudents.length) { preview.style.display = 'none'; return; }

    if (!toDeleteNow.length && !toDeleteLater.length) {
      preview.style.cssText = 'display:block;background:#d1fae5;border:1.5px solid #6ee7b7;color:#065f46;border-radius:10px;padding:.65rem 1rem;font-size:.84rem';
      preview.innerHTML = '✅ Tất cả học viên được giữ lại mãi mãi — sẽ không xóa ai.';
      return;
    }
    preview.style.cssText = 'display:block;background:#fef2f2;border:1.5px solid #fca5a5;color:#991b1b;border-radius:10px;padding:.65rem 1rem;font-size:.84rem';
    let html = '';
    if (toDeleteNow.length) html += `🗑 <b>${toDeleteNow.length}</b> học viên xóa ngay · Giữ lại <b>${_dsKeepIds.size}</b>`;
    if (toDeleteLater.length) {
      if (html) html += '<br/>';
      html += `<span style="font-size:.78rem">📅 Lên lịch: ${toDeleteLater.map(s => `<b>${s.full_name}</b> — ${new Date(_dsExpireDates[String(s.id)]).toLocaleString('vi-VN')}`).join(' · ')}</span>`;
    }
    preview.innerHTML = html;
  }

  // ---- Tìm kiếm ----
  document.getElementById('dsSearchInput')?.addEventListener('input', function() {
    renderDsStudentList(this.value.trim().toLowerCase());
  });

  // ---- Tích tất cả = giữ hết ----
  document.getElementById('dsSelectAllBtn')?.addEventListener('click', () => {
    _dsAllStudents.forEach(s => _dsKeepIds.add(String(s.id)));
    document.querySelectorAll('#dsStudentList .ds-row').forEach(row => {
      const id = row.dataset.sid;
      row.style.background = '#f0fdf4';
      const cb = row.querySelector('.ds-cb'); if (cb) cb.checked = true;
      const dc = row.querySelector('.ds-date-cell');
      if (dc) {
        const v = _dsExpireDates[id] ? new Date(_dsExpireDates[id]).toISOString().slice(0,16) : '';
        dc.innerHTML = `<input type="datetime-local" class="ds-date-input" data-id="${id}" value="${v}"
          title="Ngày xóa (để trống = giữ mãi mãi)"
          style="padding:.3rem .45rem;border:1.5px solid #6ee7b7;border-radius:7px;outline:none;font-size:.76rem;color:#065f46;background:#f0fdf4;width:100%">`;
      }
      const bc = row.querySelector('.ds-badge-cell'); if (bc) bc.innerHTML = '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#d1fae5;color:#065f46;white-space:nowrap">✓ Giữ lại</span>';
    });
    updateDsCount(); renderDsPreview();
  });

  // ---- Bỏ tích tất cả = xóa hết ----
  document.getElementById('dsSelectNoneBtn')?.addEventListener('click', () => {
    _dsKeepIds.clear(); _dsExpireDates = {};
    document.querySelectorAll('#dsStudentList .ds-row').forEach(row => {
      row.style.background = '#fff5f5';
      const cb = row.querySelector('.ds-cb'); if (cb) cb.checked = false;
      const dc = row.querySelector('.ds-date-cell'); if (dc) dc.innerHTML = '<span style="color:var(--muted);font-size:.78rem">Xóa ngay</span>';
      const bc = row.querySelector('.ds-badge-cell'); if (bc) bc.innerHTML = '<span style="font-size:.72rem;font-weight:700;padding:.2rem .5rem;border-radius:20px;background:#fee2e2;color:#991b1b;white-space:nowrap">🗑 Xóa</span>';
    });
    updateDsCount(); renderDsPreview();
  });
  ...
  document.getElementById('dsCancelBtn')?.addEventListener('click', () => {
    _dsTimers.forEach(t => clearTimeout(t)); _dsTimers = [];
    document.getElementById('deleteSelectiveModal').classList.remove('open');
  });

  // ---- Xác nhận ----
  document.getElementById('dsConfirmBtn')?.addEventListener('click', async () => {
    const now = new Date();
    const toDeleteNow   = _dsAllStudents.filter(s => !isKept(s.id));
    const toDeleteLater = _dsAllStudents.filter(s => isKept(s.id) && _dsExpireDates[String(s.id)] && _dsExpireDates[String(s.id)] > now);
    const errEl = document.getElementById('dsError');
    errEl.style.color = ''; errEl.textContent = '';

    if (!toDeleteNow.length && !toDeleteLater.length) {
      errEl.textContent = 'Không có học viên nào bị xóa.'; return;
    }

    const parts = [];
    if (toDeleteNow.length)   parts.push(`${toDeleteNow.length} xóa ngay`);
    if (toDeleteLater.length) parts.push(`${toDeleteLater.length} lên lịch`);

    showConfirm(`${parts.join(' · ')} học viên khỏi lớp "${_dsClass}"?`, async () => {
      const btn = document.getElementById('dsConfirmBtn');
      btn.disabled = true; btn.textContent = '⏳ Đang xử lý...';

      if (toDeleteNow.length) await performSelectiveDelete(toDeleteNow, _dsClass);

      for (const s of toDeleteLater) {
        const delay = new Date(_dsExpireDates[String(s.id)]).getTime() - Date.now();
        _dsTimers.push(setTimeout(() => performSelectiveDelete([s], _dsClass), Math.max(0, delay)));
      }

      btn.disabled = false; btn.textContent = '🗑 Xác nhận xóa';
      if (toDeleteLater.length > 0) {
        errEl.style.color = '#10b981';
        errEl.textContent = `✅ Đã xóa ${toDeleteNow.length} ngay. Lên lịch ${toDeleteLater.length} học viên — giữ trang mở đến đúng giờ.`;
      } else {
        document.getElementById('deleteSelectiveModal').classList.remove('open');
      }
    }, { title: '🗑 Xác nhận xóa chọn lọc', icon: '⚠️', okText: `🗑 Xóa ${toDeleteNow.length + toDeleteLater.length} học viên` });
  });

  async function performSelectiveDelete(toDelete, cls) {
    ...
    for (const s of toDelete) {
      await db.from('alerts').insert({
        student_name: s.full_name || '', username: s.username || '', class_name: cls,
        reason: `Xóa tài khoản (chọn lọc) — bởi Admin ${by}`
      });
    }
    renderClasses(); populateClassFilters(); renderStudents(); renderOverview();
  }
